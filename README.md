...
> ℹ️ Для некоторых callables (например, часть C-расширений или динамические объекты)
> сигнатура может быть недоступна или неполной. В таких случаях библиотека использует
> безопасный fallback-режим:
> - `exclude_kwargs` применяется всегда;
> - `self`/`cls` удаляются из `kwargs` при `exclude_self=True`;
> - positional аргументы не раскрываются при `exclude_self=True` (`args=<unbound>`).
