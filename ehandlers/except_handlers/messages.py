...
def err_annotated_msg(
    ...
    *,
    ...
    args_info: str | None = None

    # 1) Основной путь: полная привязка; 2) fallback привязки: partial.
    if cached_sig is not None:
        binder: Callable
        for binder in (cached_sig.bind, cached_sig.bind_partial):
            try:
    ...
    if args_info is None:
        ...
        if exclude_self:
            clean_kwargs.pop('self', None)
            clean_kwargs.pop('cls', None)
        for key in excluded:
        ...
        if exclude_args:
            ...
            # Гарантированно отделить `self/cls` из positional args нельзя.
            args_repr = '<unbound>'
        else:
            args_repr = repr(args)
